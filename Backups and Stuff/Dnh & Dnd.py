...
d = str(input("Dnd or Dnh? (Case sensitive) "))
n = int(input("n = "))
terms = []
...
# Metallocene-like Dnh for n = 2
if n == 2 and d == "Dnh":
    print("2x^4_1, 6x^2_2")

# Metallocene-like Dnh for even "n"
if n % 2 == 0 and n > 2 and  d == "Dnh":
 
    #Improper Rotations (S)

    def improper_top_function(x):
        if (n/math.gcd(n,x)) % 2 == 0:
            return int(2*math.gcd(n,x))
        elif 2 > ((n/math.gcd(n,x))) % 2 > 0:
            return int(math.gcd(n,x))
        else:
            return 0
    ...
    terms.append(f"{int(((3*n)/2)+3)}x^{n}_{2}")

    start_range = 1
    end_range = int((n/2)-1)
    term_2_range = int(((3*n)/2)+3)
    improper_top_values = calculate_improper_top_values(start_range, end_range)
    proper_top_values = calculate_proper_top_values(start_range, end_range)
    improper_bottom_values = calculate_improper_bottom_values(start_range, end_range)
    ...
    print(combined_terms, f"{int(n/2)}x^{4}_{1} x^{n-2}_{2}")

# Metallocene-like Dnd for even "n"
if n % 2 == 0 and d == "Dnd":
...
# Metallocene-like Dnh & Dnd for odd "n"
if n % 2 == 1:
    ...
    term_2_range = int(((3*n)/2)+3)
    improper_top_values = calculate_improper_top_values(1, n-2)
    proper_top_values = calculate_proper_top_values(1, int((n-1)/2))
    improper_bottom_values = calculate_improper_bottom_values(1, n-2)
    proper_bottom_values = calculate_proper_bottom_values(1, int((n-1)/2))
